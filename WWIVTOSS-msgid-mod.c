        ...
        switch (ch)
        {
            ...
            case '\r':  /* End of line */
                ...
                if (!strnicmp(temp,"0MSGID: ",9))
                {
                   ...
                   /* Let's try to get address from INTL */
                if ((!strnicmp(temp,"0INTL: ",8)) && (!msgid))
                {
                    strcpy(origi_node,&temp[8]);
                    sscanf(origi_node,"%s %s",t1,msgidd);
                    msgid2=1;
                    msgid=1;  /* MMH mod */
                }
...
void bounce_mail(void)
{
    ...
    if (out_zone!=my_zone)
    {
        sprintf(s,"INTL %d:%d/%d.%d %d:%d/%d.%d\r",out_zone,out_net,out_node,out_point,my_zone,my_net,my_node,my_point);
        strcat(mess,s);
    }
