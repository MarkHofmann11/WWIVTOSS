...
unsigned char *translate_letters[] = {
  "abcdefghijklmnopqrstuvwxyzáÑÜÇÅî§",
  "ABCDEFGHIJKLMNOPQRSTUVWXYZÄéèêöô•",
  0L,
};
...
void process_outgoing(void)
{
    ...
    for (cycle1=0;cycle1<11;cycle1++)
    {
        if (cfg.aka_list[cycle1].zone!=0)
        {
            ...
            {
                printf("˛ Checking for Pending Messages on %s\r\n",net_networks[net_num].name);
                sprintf(log,"˛ Checking for Pending Messages (%s) on %s\n",netfile,net_networks[net_num].name);
                write_log(log);
            }
            if (!exist(netfile))
            {
                if (oldnetnum!=net_num)
                {
                    printf("˛ No Outgoing Messages for %s\r\n",net_networks[net_num].name);
                    sprintf(log,"˛ No Outgoing Messages for %s\n",net_networks[net_num].name);
                    write_log(log);
                }
                oldnetnum=net_num;
            }
            else
            {
                ...
                sprintf(log,"˛ Processing %s to %s\n\n",netfile,net_networks[net_num].name);

                write_log(log);
                if (!in)
                {
                    printf("\r\n\7˛ Error!  Couldn't open %s for processing!\r\n",netfile);
                    sprintf(log,"\n˛ Error!  Couldn't open %s for processing!\n",netfile);
                    write_log(log);
                    return;
                }
                ...
                while(1)
                {
                    ...
                    if ( (temp.main_type==5) || (temp.main_type==3) || (temp.main_type==26))  /* Message is a post! */
                    {
                        ...
                        if (!message)
                        {
                            printf("˛˛ Error: Couldn't Allocate Memory!\7\n\n");
                            write_log("˛ Error!  Couldn't Allocate Memory to export message!\n");
                            exit(1);
                        }
...
*/
                            ...
                            if (ok2)
                            {
                                ...
                                {
                                    flag++;
                                    if ((flag==1) && (cfg.initial_quote) && ((ch=='>') || (ch=='Ø')))
                                    {
                    ...
                    if (temp.main_type==7)  /* Message is email! */
                    {
                     ...
                     /* if (out_zone!=my_zone) */
                        /* { */
                            sprintf(s3,"INTL %d:%d/%d.%d %d:%d/%d.%d\r",out_zone,out_net,out_net,out_node,my_zone,my_net,my_node,my_point);
                            strcat(message,s3);
                       /* } */
                      /* End of update */ 
...
//                                printf("Using External Origin Line from %d\r\n",ext_fn);
                        ...
                        {
                            bad_mail++;
                            sprintf(s3,"˛ Invalid address on outgoing mail from %u@%u\n",temp.fromuser,temp.fromsys);
                            write_log(s3);
...
//                                    write_log(log);
                ...
                fsh_close(in);
                printf("˛ Processing on %s finished!\r\n\r\n",net_networks[net_num].name);
                unlink(netfile);
                outchr(00);
...
void pack_outbound_packets(void)
{
    ...
    while ((f1==0) && (exit_status==0))
    {
        ...
        if (incoming_file<0)
        {
            printf("\7˛ Error opening outbound packet: %s. errno=%d\n",s,errno);
            sprintf(log,"˛ Error opening outbound packet: %s. errno=%d\n [%s]",s,errno,sys_errlist[errno]);
            write_log(log);
            rename(s,old_name);
            exit_status=6;
            break;
        ...
        if (compress>0)
        {
            ...
            if (unsuccessful)
            {
                if (unsuccessful<0)
                {
                    printf("˛ Error with spawn!  Error= %s\r\n",sys_errlist[errno]);
                    sprintf(log,"˛ Error with spawn!  Error= %s\n",sys_errlist[errno]);
                    write_log(log);
                }
                else
                {
                    printf("˛ Error Archiving Packet! (%s error code %d)\r\n",cfg.archive[compress].name,unsuccessful);
                    sprintf(log,"˛ Error Archiving Packet! (%s error code %d)\n",cfg.archive[compress].name,unsuccessful);
                    write_log(log);
                }
                rename(s,old_name);
                return;
        ...
        if (ok2==2)
        {
            ...
            if (cfg.mailer==2)
            {
                ...
                sprintf(log,"˛ Creating File Attach message %s\n",fn);
                write_log(log);
                if (attach_file != NULL)
                {
...
void return_message(net_header_rec temp, char *message)
{
    ...
    strcpy(mess,"6˛2 This Message is being returned to you because the addressing was\r\n");
    strcat(mess,"6˛2 incorrect.  Please check your setup and try again.\r\n");
    sprintf(s,"6˛1 Mail formatting for Fidonet is USERNAME (FIDO_NODE) @%u\r\n",nh.fromsys);
    strcat(mess,s);
